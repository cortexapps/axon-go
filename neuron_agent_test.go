...
import (
	...
	"github.com/cortexapps/neuron-go/mock_neuron"
...
func createAgent(controller *gomock.Controller) (*Agent, *mockGrpcClient) {
	agent := NewNeuronAgent(WithSleepOnError(0))
	agent.client = &mockGrpcClient{
		apiStub:   mock_neuron.NewMockCortexApiClient(controller),
		agentStub: mock_neuron.NewMockNeuronAgentClient(controller),
	}
...
type mockGrpcClient struct {
	apiStub   *mock_neuron.MockCortexApiClient
	agentStub *mock_neuron.MockNeuronAgentClient
}
...
func (m *mockGrpcClient) agent() pb.NeuronAgentClient {
	return m.agentStub
}
